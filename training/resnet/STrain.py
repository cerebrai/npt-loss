...
class Trainer(object):
    ...
    def __init__(self, model, Param, loss_f, checkpoint={}, save_dir=None, save_freq=2, resume_flag=False):
        ...
        if not resume_flag:
            self.head = Linear(512, self.checkpoint["num_classes"], multi_flag=self.multi_flag, device_id=self.device_id)
            #self.head = ArcFace(256, self.checkpoint["num_classes"], s = self.r, m=self.delta, device_id=self.device_id)
        else:
            self.head = Linear(512, self.checkpoint["num_classes"], fc=self.checkpoint["head_fc"], multi_flag=self.multi_flag, device_id=self.device_id)
        ...
        # -- Defining optimizer
        param1 = [param for param in self.model.parameters()]
        param2 = [param for param in self.head.parameters()]
        self.optimizer = optim.SGD(params=param1 + param2, lr=Param['lr'], momentum=Param['moment'], weight_decay=Param['decay'])
